...
class DesignBot:
    ...
    def __init__(self):
        ...
        # Use FLUX.2 for superior image generation
        self.model_name = "black-forest-labs/FLUX.1-dev"
        
        logger.info(f"Initialized {self.name} with FLUX.2 Model")
    ...
    async def generate_logo(
        ...
        prompt = self._build_logo_prompt(brand_name, style, colors)
        ...
        try:
            if self.hf_token:
                ...
                if image_bytes:
                    ...
                    logger.success(f"\u2705 {self.name} generated logo via FLUX.2")
                    return {
            ...
            logger.debug(f"FLUX.2 failed: {str(e)}, using placeholder")
        
        # Use placeholder logo
        logger.warning("Using placeholder logo")
    ...
    def _build_logo_prompt(
        ...
        using {color_str} colors, clean, minimalist, vector art, 
        high quality, white background, centered composition"""
        ...
        try:
            async with httpx.AsyncClient(timeout=90.0) as client:
                ...
                if response.status_code == 200:
                    logger.success(f"✅ Image API success ({len(response.content)} bytes)")
                    return response.content
                else:
                    logger.debug(f"Image API returned status {response.status_code}")
        except Exception as e:
            logger.debug(f"Image API error: {str(e)}")
    ...
    def _create_placeholder_logo(
        ...
        # Parse color (handle both hex and names)
        ...
        shadow_offset = 5
        draw.text((center + shadow_offset, center + shadow_offset), initial, 
                 fill=(0, 0, 0, 100), font=font, anchor="mm")
        draw.text((center, center), initial, fill='white', font=font, anchor="mm")
